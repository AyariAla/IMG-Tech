...
export class CategorysRepository extends Repository<Category>{
    ...
    async getCategory(filterDto: GetCategorysFilterDto/*later add category in product*/):Promise<Category[]>{
        const {search}=filterDto;
        const querry = this.createQueryBuilder('category')
        //later in product add category to return product belong tp that category query.where
        if (search){
        querry.andWhere(
            'LOWER(category.name) LIKE LOWER(:search) OR LOWER(category.name) LIKE LOWER(:search)',
            {search:`%${search}$` },
        );
        }
        const categorys = await querry.getMany();
        return categorys ;
    }
